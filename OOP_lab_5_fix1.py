...
class User:
    def __init__(self, name, login, password, database_main):
        self.__name = name
        self.__dbname = database_main #название бд в которой хранится пользователь
        self.__login = login
        self.__password = password
        self.__autorisatiion = False #состояние авторизованности пользователя
        self.__state = False            #состояние пользователя - зашел или вышел из ЛК
...
class User_Repositiry(IUser_Repositiry):
    def __init__(self, filename="users_data.pkl"):
        self.filename = filename
        self.__dbnameUsers = self.load_from_file()
    ...
    def save_to_file(self):
        with open(self.filename, 'wb') as file:
            pickle.dump(self.__dbnameUsers, file)
...
def Main_Cycle():
    ...
    testuser = UserManager("Alex", "AlexFedo", "123pass", database_main)
